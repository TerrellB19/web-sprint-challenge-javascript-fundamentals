...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// The reason that external can be accessed within the function is that its not blocked by any scope. scopes kinda work like a secret society in the way that everything inside of the function is aware of what exists outside of it, but the things on the outside arent aware that anything iside actually exsists.
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(arr){
    const array = [];
    const lowerCase = arr.map(element => {
      array.push(element.animal_name.toLowerCase())
    })
    return array
  }
  console.log('task 1, request 2:', lowerCaseNames(zooAnimals))
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    const popFilter = arr.filter((element) => {
      return element.population < 5
    })
    return popFilter
  }
  console.log('task 1, request 3:', lowPopulationAnimals(zooAnimals))
  ...
  */
  ...
  function USApop(arr){
    const reducedZoo = arr.reduce((accumulator, currentValue) => {
      return accumulator + currentValue.population
    }, 0)
    return reducedZoo
  }
  console.log('task 1, request 4:', USApop(zooAnimals))
  ...
  */
  ...
  function consume(paramA, paramB, cb ){
    return cb(paramA, paramB)
  }
